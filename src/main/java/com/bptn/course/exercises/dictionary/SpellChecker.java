package com.bptn.course.exercises.dictionary;
...
public class SpellChecker {
	...
	public SpellChecker() throws IOException {
		...
		List<String> lines = Files.readAllLines(Paths.get(
				"C:\\Users\\kanyi\\source\\workspace-academy-java\\academy-java\\src\\main\\java\\com\\bptn\\course\\exercises\\dictionary\\dictionary.txt"));
	...
	public boolean linearSpellCheck(String word) {
		...
		for (int i = 0; i < dictionary.size(); i++) {
			...
			if (word.equals(dictionary.get(i))) {
				System.out.println("Number of words checked Linear: " + count);
				return true;
			}
		}
		System.out.println("Number of words checked Linear: " + count);
	...
	public boolean binarySpellCheck(String word) {
		...
		while (left <= right) {
			...
			if (word.compareTo(dictionary.get(middle)) < 0) {
				...
				System.out.println("Number of words checked Binary " + count);
				return true;
			}
